...
class Fluid_walls(SubDomain):
        def inside(self,x,on_boundary):
                return (abs(x[0]) > x1 - tol or x[1] > h-tol) and on_boundary 
...
class Fluid_in_l(SubDomain):
        def inside(self,x,on_boundary):
                return x[1]>(1.2*h-tol) and on_boundary and x[0] < - x0 + tol

class Fluid_in_r(SubDomain):
		def inside(self,x,on_boundary):
				return x[1]>(1.2*h-tol) and on_boundary and x[0] > x0 - tol

class Top_walls(SubDomain):
		def inside(self,x,on_boundary):
			return x[1] > h-tol and x[1] < 1.1*h + tol and on_boundary

class Solid_in(SubDomain):
        def inside(self,x,on_boundary):
                return abs(x[1]-h)<tol and on_boundary and abs(x[0]) < x0 + tol

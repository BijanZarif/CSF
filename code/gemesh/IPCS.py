...
test = Expression(('x[0]*0.00001*x[1]*cos(2*pi*t)','0.0'),t=0)
...
bcd1 = DirichletBC(V,test,boundaries,2)
...
ufile = File('results_IPCS/vel.pvd')
pfile = File('results_IPCS/pr.pvd')
dfile = File('results_IPCS/disp.pvd')
...
F1 = (1/k)*inner(u - u0, v)*dx(0, subdomain_data=subdomains) + \
     inner(grad(u0)*u0, v)*dx(0, subdomain_data=subdomains) + \
     nu_f*inner(grad(u), grad(v))*dx(0, subdomain_data=subdomains) - \
     inner(f, v)*dx(0, subdomain_data=subdomains) + \
     mu_s*inner(grad(d),grad(v))*dx(1,subdomain_data = subdomains) + \
     (lamda_s+mu_s)*inner(div(d),div(v))*dx(1,subdomain_data=subdomains) + \
     inner(p,div(v))*dx(1,subdomain_data=subdomains)
...
# Pressure update
a2 = inner(grad(p), grad(q))*dx(0, subdomain_data=subdomains) +  \
     inner(grad(p),grad(q))*dx(1, subdomain_data=subdomains) 
L2 = -(1/k)*div(u1)*q*dx(0, subdomain_data=subdomains) + \
     inner(Constant('0.0'),q)*dx(1, subdomain_data=subdomains)

# Velocity update
a3 = inner(u, v)*dx(0, subdomain_data=subdomains) + \
     inner(grad(u),grad(v))*dx(1, subdomain_data=subdomains)
L3 = inner(u1, v)*dx(0, subdomain_data=subdomains) - \
     k*inner(grad(p1), v)*dx(0, subdomain_data=subdomains) + \
     inner(Constant(('0','0')),v)*dx(1, subdomain_data=subdomains)


# Tentative 

a4 = mu_s*inner(grad(d),grad(v))*dx(1,subdomain_data=subdomains) + \
     (mu_s+lamda_s)*inner(div(d),div(v))*dx(1,subdomain_data=subdomains) + \
     inner(div(d),q)*dx(1,subdomain_data=subdomains) + \
     mu_f*inner(grad(d),grad(v))*dx(0,subdomain_data=subdomains)

L4 = (1.0/lamda_s)*inner(u,v)*dx(1,subdomain_data=subdomains) - \
     inner(u,grad(q))*dx(1,subdomain_data=subdomains) + \
     inner(Constant(('0.0','0.0')),v)*dx(0,subdomain_data=subdomains) + \
     inner(Constant(('0.0','0.0')),v)*dx(1,subdomain_data=subdomains) + \
     Constant('0')*q*dx(0,subdomain_data=subdomains) + \
     Constant('0')*q*dx(1,subdomain_data=subdomains)
# Assemble matrices
